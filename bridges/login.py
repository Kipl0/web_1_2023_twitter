...
def _():
    ...
    try:
        ...
        does_user_exist = db.execute("SELECT * FROM users WHERE user_username = ? LIMIT 1",(login_username,)).fetchall()[0]

        if bcrypt.checkpw(login_password, does_user_exist["user_password"]) : 
            ...
            return {"info":"success log n", "login_username":does_user_exist["user_username"]}
        else :
