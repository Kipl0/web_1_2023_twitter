...
def _(tweet_id):
  try:
    ...
    tweet_to_view = db.execute("SELECT * FROM tweets WHERE tweet_id = ? COLLATE NOCASE",(tweet_to_view,)).fetchall()[0]
    tweet_to_view_id = user["user_id"]    
    ...
    who_to_follow = []
    who_to_follow = db.execute("SELECT * FROM users WHERE user_username!=? AND user_username != ?",(user_cookie['user_name'],"Admin"))
    ...
    return template("view_tweet")
    # return template("profile", tweets_and_user_data=tweets_and_user_data, trends=trends, user_cookie=user_cookie, who_to_follow=who_to_follow)
